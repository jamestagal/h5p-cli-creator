...
Extract only the relevant portion of a video to reduce Whisper API transcription costs by skipping intros, outros, or irrelevant content.
...
**Cost savings:** For a 20-minute video, trimming to 17 minutes (skip 2-min intro + 1-min outro) saves $0.02 per video (17% reduction).

**Supported time formats:** `MM:SS` (e.g., "01:30") or `HH:MM:SS` (e.g., "01:30:00")

**Important:** When using time ranges, page timestamps become relative to the extracted audio. Page `startTime: "00:00"` corresponds to `source.startTime` in the original video. See examples for details.

**Backward compatibility:** Omitting `startTime` and `endTime` extracts the full video (default behavior).
...
- **Whisper transcript**: `.youtube-cache/VIDEO_ID/whisper-transcript.json`
- **Translations**: `.youtube-cache/VIDEO_ID/translations.json`
- **Cache metadata**: `.youtube-cache/VIDEO_ID/cache-metadata.json` (includes extraction range, cost info)
- **Audio segments**: `.youtube-cache/VIDEO_ID/audio-segments/` (page1.mp3, page2.mp3, etc.)

**Cache organization:** All assets for a video are co-located in a single directory:
...
```yaml
# Valid:
startTime: "01:30"     # MM:SS
startTime: "01:30:00"  # HH:MM:SS

# Invalid:
startTime: "90"        # Must include colons
startTime: "1:30"      # Minutes must be zero-padded
```
...
**Solution:** Ensure startTime is before endTime in `source` config.

### Error: "endTime exceeds video duration"

**Cause:** Specified endTime is beyond the video length

**Solution:** Check video duration and ensure `source.endTime` does not exceed it.

### Error: "Page N endTime exceeds trimmed audio duration"

**Cause:** Page timestamp exceeds the extracted audio duration when using time range extraction

**Solution:** Page timestamps must be relative to trimmed audio, not original video. If `source.startTime: "01:30"` and `source.endTime: "10:00"`, the trimmed duration is 08:30, so page endTime cannot exceed "08:30".
