...
Both review tables are implemented from phase 2 as revision-bound records in the `ImportStore`, written by `leap review`; the web UI arrives in phase 5.
...
- **URL fetching**: DNS resolution is checked against private, loopback, link-local and metadata address ranges before connecting and again on every redirect; redirects are capped; response size, content type and time are limited. Implemented as `safeFetch` in `packages/generator` (phase 2): addresses are classified after IPv6 normalisation and the connection is pinned to the validated address; used by every application-side fetch.
