...
**Preview** uses `h5p-standalone` on a **separate preview origin** (a distinct hostname, e.g. `preview.<host>`), which serves extracted packages from R2 under a narrowly scoped, short-lived access token bound to `(org, activity revision)`. The preview origin sets no application cookies and receives none. The application embeds it in an iframe whose `sandbox` value is decided by the **phase-1 preview spike**: the target is `allow-scripts` alone, but without `allow-same-origin` the document has an opaque origin, which affects `fetch` of package JSON and media and any nested H5P frames. The spike tests the exact token and CORS arrangement with real packages; if `allow-scripts` alone cannot work, `allow-scripts allow-same-origin` on the cookie-less separate origin is the accepted fallback, and the decision is recorded in the spec.
