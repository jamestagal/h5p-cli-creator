...
|---|---|---|
| Preserve legacy narrated audio books (owner decision, 19 Sep 2026) | **Do not delete `apps/cli-legacy`.** Keep its existing commands, source, configurations, examples and tests. Narrated audio books are a distinct Interactive Book composition mode, with a source-video introduction, ordered text/image/audio pages and optional translations; see [design §6.2](../specs/2026-09-18-generator-service-design.md#62-narrated-audio-book-preservation). No migration is added to phase 2. | Generated activity collections do not replace the existing reading/listening workflow. A later replacement needs explicit page/media/timing/language parity; even passing parity does not authorize deleting the folder. |
...
- **Legacy preservation:** do not delete `apps/cli-legacy` or remove its existing command entry points, examples, configurations, tests or source as cleanup. The narrated audio-book contract is distinct from activity-collection books (design §6.2). Phase 4 or phase 6 completion is not deletion authorization; only a separate explicit instruction from Benjamin can authorize future removal.
...
- **Existing work preserved:** `apps/cli-legacy` and its command entry points remain available; its regression suite still passes. The YouTube/narrated audio-book workflow is retained as specified in design §6.2. This phase adds no narrated-book migration or legacy retirement task.
...
- **Legacy retention and narrated-book mode:** the owner clarified on 19 Sep 2026 that `apps/cli-legacy` must not be deleted. Design §§2.1a, 6.2 and 11 now supersede any older phase-4 deletion wording. Future migration must preserve the narrated-book page structure, audio/image associations, timestamps and optional translations; this is separate from generating an activity-collection book. No phase-2 runtime work is added by this clarification.
