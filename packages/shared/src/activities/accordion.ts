...
export const AccordionSpec = ActivityBase.extend({ type: z.literal("accordion"), panels: z.array(AccordionPanel).min(1).max(30) }).superRefine((s, ctx) => rejectDuplicateIds(s.panels, ["panels"], ctx));
